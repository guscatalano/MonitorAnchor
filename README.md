...
| When I change display settings myself | A change with the same monitors connected, while you are at the keyboard, and not made by the app, is yours. **Update the saved layout** (default) saves it after 20 s, once Windows' own "Keep these settings?" prompt has had its chance to revert; **Ask me** shows a notification you click to save; **Put the saved layout back** is the old behaviour. Plug/unplug, resume, unlock and startup are always enforced. |
...
| When idle ▸ Count as idle after | 30 s, 1 min (default), 2, 5 or 10 minutes without keyboard or mouse input. |
| Windows Update ▸ | Shows whether updates are paused and until when. Pause for 1 week, 5 weeks, 6 months or 1 year, or resume. Each action relaunches the app elevated for the registry write, so Windows asks for administrator approval. Writes the same pause window the Settings page does. |
